...
std::tuple<CryptoContext<DCRTPoly>, KeyPair<DCRTPoly>, int> serverSetupAndWrite(int multDepth, int scaleModSize,
    ...
    vector<double> images_X;
    for (unsigned k = 0; k < n_images*784; ++k){
            images_X.push_back(X_train[k]);
    }

    Plaintext serverP_X_train = serverCC->MakeCKKSPackedPlaintext(images_X);
    std::cout << "Plaintexts have been generated from train.txt" << std::endl;

    auto serverC_X_train = serverCC->Encrypt(serverKP.publicKey, serverP_X_train);
    std::cout << "Ciphertexts have been generated from train.txt\n" << std::endl;
