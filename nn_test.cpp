...
vector <float> sigmoid_d (const vector <float>& m1) {
    ...
    const long VECTOR_SIZE = m1.size();
...
vector <float> operator*(const float m1, const vector <float>& m2){
    ...
    const long VECTOR_SIZE = m2.size();
    vector <float> product (VECTOR_SIZE);
...
int main(int argc, const char * argv[]) {
    ...
    for (int i = 0; i < 10000; ++i) {
        ...
        vector<float> b_X;
        vector<float> b_y;
        for (int j = randindx*784; j < (randindx+BATCH_SIZE)*784; ++j){
            b_X.push_back(X_train[j]);
        }
        for (int k = randindx*10; k < (randindx+BATCH_SIZE)*10; ++k){
